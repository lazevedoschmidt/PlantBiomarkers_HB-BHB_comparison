...
dleaf_plot <- dleaf_dat %>%
  ...
  labs(x="Epoch", y=expression(Delta[leaf]))+
  ggtitle("Leaf fractionation during Paleocene and Eocene")+
  theme_light()
...
#############################################################################
#saving figure
ggsave("Figures/dleaf_basin_boxplot.pdf", dleaf_plot, width = 6.5, scale = 1)
